...
namespace MatlabFileIO
{
    ...
    public class MatFileArrayWriter<T> : IMatlabFileWriterLocker
    {
        ...
        public MatFileArrayWriter(string varName, BinaryWriter writeStream)
        {
        ...
        public void AppendData(T[] dataToAppend)
        {
            ...
            dataLength += dataToAppend.Length * MatlabBytesPerType<T>();

            //needed for array dimensions
            firstDim++;
        }

        private int MatlabBytesPerType<T>()
        {
            int bytesPerData = 0;
